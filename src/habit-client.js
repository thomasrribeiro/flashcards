...
export async function getSettings() {
    ...
    accountSettingsCache = { ...DEFAULT_SETTINGS, ...settings };
    return accountSettingsCache;
}

/**
 * Save habit settings (partial update). Signed-in state is persisted only in
 * D1; localStorage belongs exclusively to the signed-out profile.
 */
export async function saveSettings(partial) {
    ...
    accountSettingsCache = {
        ...(accountSettingsCache || DEFAULT_SETTINGS),
        ...partial
    };
    return enqueueSettingsPersistence(id, partial);
}

function enqueueSettingsPersistence(id, partial) {
...
export async function getHabitStatus(now = new Date()) {
    ...
    if (id) {
        ...
        accountSettingsCache = status.settings;
        return status;
    }
