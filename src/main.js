...
async function init() {
    ...
    try {
        ...
        const initialStudyTarget = studyTargetFromUrl();
        ...
        if (initialStudyTarget) await applyStudyTarget(initialStudyTarget);
        ...
        } else if (initialStudyTarget) {
            writeStudyHistory({ replace: true });
        }

        // Study's deck finder uses a different data source by account state.
...
function applyTheme(theme) {
    ...
    if (button) {
        ...
        button.innerHTML = dark
            ? '<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="3.5" fill="none" stroke="currentColor" stroke-width="1.7"/><path d="M12 2v2.2M12 19.8V22M4.93 4.93l1.56 1.56M17.51 17.51l1.56 1.56M2 12h2.2M19.8 12H22M4.93 19.07l1.56-1.56M17.51 6.49l1.56-1.56" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="square"/></svg>'
            : '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20 15.2A8.5 8.5 0 0 1 8.8 4 8.5 8.5 0 1 0 20 15.2Z" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linejoin="round"/></svg>';
...
function closeChapterBrowser() {
    curriculumChapterPreviewGeneration += 1;
    const modal = document.getElementById('card-browser-modal');
...
function curriculumDeckHasGeneratedCards(deck) {
...
function openCurriculumDeckActionsModal({ deck, registry, installedRepository }, trigger = null) {
    ...
    const path = document.getElementById('deck-actions-path');
    const body = document.getElementById('deck-actions-body');
    if (!modal || !title || !path || !body || !deck) return;

    activeDeckActions = { curriculumDeck: deck };
    activeDeckActionsTrigger = trigger;
    title.textContent = deck.deck || deck.id.split('/').pop();
    path.textContent = `~ / ${registry?.repository || 'curriculum'} / ${deck.subject} / ${deck.deck || deck.id}`;
    body.innerHTML = '';
    const actions = [];
    ...
    actions.push(appendDeckAction(body, {
        label: 'Prereqs & unlocks',
        description: 'Inspect the decks required before and unlocked after this deck.',
        onClick: () => {
            closeDeckActionsModal({ restoreFocus: false });
            navigateCurriculum({
                mode: 'focus', hierarchy: 'deck', subject: deck.subject,
                parentId: deck.subject, targetId: deck.id, query: ''
            });
        }
    }));
...
function studySelectionTarget() {
    return {
        subjectId: columnsSel.subject || '',
        repositoryId: columnsSel.deck || '',
        chapterFile: columnsSel.chapter || ''
    };
}

function studyTargetFromUrl(url = new URL(window.location.href)) {
    if (url.searchParams.get('view') !== 'study') return null;
    return {
        subjectId: url.searchParams.get('study-subject') || '',
        repositoryId: url.searchParams.get('study-deck') || '',
        chapterFile: url.searchParams.get('study-chapter') || ''
    };
}

function writeStudyHistory({ replace = false } = {}) {
    const url = new URL(window.location.href);
    const target = studySelectionTarget();
    url.searchParams.set('view', 'study');
    for (const key of [
        'curriculum-mode', 'curriculum-level', 'curriculum-target',
        'curriculum-parent', 'curriculum-subject', 'curriculum-layer', 'recommended'
    ]) url.searchParams.delete(key);
    const params = [
        ['study-subject', target.subjectId],
        ['study-deck', target.repositoryId],
        ['study-chapter', target.chapterFile]
    ];
    ...
    history[replace ? 'replaceState' : 'pushState'](
        { mainView: 'decks', studyTarget: target },
        '',
        `${url.pathname}${url.search}${url.hash}`
    );
}

async function applyStudyTarget(target) {
    if (!target) return;
    const decks = await getAllDecks();
    const requestedRepository = target.repositoryId || '';
    const deck = requestedRepository
        ? decks.find(item => item.id === requestedRepository)
        : null;
    const subject = target.subjectId || subjectSlug(deck?.subject);
    const chapter = deck && target.chapterFile
        ? target.chapterFile
        : null;
    columnsSel = {
...
function installedRepositoryForCurriculumDeck(decks, curriculumDeck) {
    if (!curriculumDeck) return null;
    const repositoryId = repositoryIdForCurriculumDeck(curriculumDeck);
    return decks.find(deck => (
        deck.curriculumId === curriculumDeck.id
        || (repositoryId && deck.id === repositoryId)
        || `${subjectSlug(deck.subject)}/${String(deck.id || '').split('/').pop()}` === curriculumDeck.id
    )) || null;
}

function curriculumContextForStudySelection() {
    if (!curriculumIndex) return null;
    const target = studySelectionTarget();
    const curriculumDeck = target.repositoryId
        ? curriculumDeckForRepository(target.repositoryId, target.subjectId)
        : null;
    if (curriculumDeck) {
        const chapter = target.chapterFile
            ? chapterForFile(curriculumIndex, curriculumDeck.id, target.chapterFile)
            : null;
        const chapterTargetId = chapter ? `${curriculumDeck.id}#${chapter.id}` : '';
        return {
            mode: 'chapters',
            hierarchy: 'chapter',
            subject: curriculumDeck.subject,
            parentId: curriculumDeck.id,
            targetId: chapterTargetId,
            query: '',
            layerStart: 0,
            anchorId: chapterTargetId
        };
    }
    if (target.subjectId && curriculumIndex.decks.some(deck => deck.subject === target.subjectId)) {
        return {
            mode: 'subject', hierarchy: 'deck', subject: target.subjectId,
            parentId: target.subjectId, targetId: '', query: '', layerStart: 0,
            anchorId: ''
        };
    }
    return {
        mode: 'overview', hierarchy: 'subject', subject: '', parentId: '',
        targetId: '', query: '', layerStart: 0, anchorId: ''
    };
}

function applyCurriculumContextFromStudy() {
    const next = curriculumContextForStudySelection();
    if (!next) return;
    const previous = curriculumStateSnapshot();
    if (curriculumNavigationKey(previous) !== curriculumNavigationKey(next)) {
        recordCurriculumNavigation(previous, next);
    }
    Object.assign(curriculumViewState, next, { position: null });
}

function studyTargetForCurriculumContext() {
    if (!curriculumIndex) return null;
    const { mode, hierarchy, subject, parentId, targetId } = curriculumViewState;
    const deckId = hierarchy === 'chapter'
        ? parentId
        : hierarchy === 'deck' && mode === 'focus'
            ? targetId
            : '';
    const curriculumDeck = deckId ? curriculumMaps(curriculumIndex).decks.get(deckId) : null;
    const chapter = hierarchy === 'chapter' && targetId.includes('#')
        ? curriculumMaps(curriculumIndex).chapters.get(targetId)
        : null;
    return {
        subjectId: curriculumDeck?.subject || subject || '',
        curriculumDeck,
        chapterFile: chapter?.file || ''
    };
}

async function applyStudyContextFromCurriculum() {
    const target = studyTargetForCurriculumContext();
    if (!target) return;
    const decks = await getAllDecks();
    const repository = installedRepositoryForCurriculumDeck(decks, target.curriculumDeck);
    columnsSel = {
        subject: target.subjectId || null,
        deck: repository?.id || null,
        chapter: repository && target.chapterFile ? target.chapterFile : null
    };
}

function updateStudyRoadmapAction() {
    const button = document.getElementById('study-view-curriculum');
    if (!button) return;
    const context = curriculumContextForStudySelection();
    const hasContext = Boolean(
        context
        && (context.subject || context.parentId || context.targetId)
    );
    button.classList.toggle('hidden', !hasContext);
    button.disabled = !hasContext;
}

async function navigateStudySelection(next, { replace = false } = {}) {
    columnsSel = next;
    writeStudyHistory({ replace });
    await loadRepositories({ refreshCollection: false });
}
...
function renderColumnsView(displayDecks, allCards, allReviews, allChapterProgress, searchTerm, grid, scroll = {}) {
    ...
    for (const deck of displayDecks) {
        ...
        const curriculumDeck = curriculumDeckForRepository(deckId, subject);
        const plannedChapters = new Map((curriculumDeck?.chapters || [])
            .filter(chapter => chapter.file)
            .map(chapter => [chapter.file, chapter]));
        const loadedFileSet = new Set(loadedFiles);
        for (const file of new Set([...metadataFiles, ...loadedFiles])) {
            const planned = plannedChapters.get(file);
            if (planned && Number(planned.card_count || 0) === 0 && !loadedFileSet.has(file)) continue;
            if (!files.has(file)) files.set(file, []);
        }
    ...
    const p1 = visibleSubjectNames
        .map(subject => {
            ...
            return colRow({
                ...
                onClick: () => navigateStudySelection({ subject, deck: null, chapter: null })
            });
        });
    ...
    if (columnsSel.subject && subjects.has(columnsSel.subject)) {
        ...
        p2 = deckIds.map(deckId => {
            ...
            return colRow({
                ...
                onClick: () => navigateStudySelection({
                    subject: columnsSel.subject,
                    deck: deckId,
                    chapter: null
                })
    ...
    if (columnsSel.subject && columnsSel.deck && subjects.get(columnsSel.subject)?.has(columnsSel.deck)) {
        ...
        p3 = fileList.map(file => {
            ...
            const selectChapter = () => {
                columnsSel = { subject: columnsSel.subject, deck: deckId, chapter: file };
                writeStudyHistory();
                updateStudyRoadmapAction();
            };
            const review = () => {
                selectChapter();
                return startScopedReview(c => (c.source?.repo || c.deckName) === deckId && c.source?.file === file, chName, ['home', columnsSel.subject, deckName, chName], [deckId], [{ repo: deckId, path: file }]);
            };
...
function setupEventListeners() {
    ...
    document.getElementById('study-view-curriculum')?.addEventListener('click', () => (
        navigateMainView('curriculum')
    ));
...
async function navigateMainView(view) {
    ...
    const previousView = currentMainView;
    if (view === 'curriculum' && previousView === 'decks') {
        applyCurriculumContextFromStudy();
    } else if (view === 'decks' && previousView === 'curriculum') {
        await applyStudyContextFromCurriculum();
    }
    ...
    for (const key of [
        ...
        'curriculum-parent', 'curriculum-subject', 'curriculum-layer', 'recommended',
        'study-subject', 'study-deck', 'study-chapter'
    ]) url.searchParams.delete(key);
...
function repositoryIdForCurriculumDeck(deck) {
    const match = /^https:\/\/github\.com\/([^/]+\/[^/#]+)\/?$/i.exec(deck?.repository?.url || '');
    return match?.[1] || '';
}

let curriculumChapterPreviewGeneration = 0;

async function loadCurriculumChapterPreview(curriculumDeck, chapter) {
    const repositoryId = repositoryIdForCurriculumDeck(curriculumDeck);
    const [owner, repository] = repositoryId.split('/');
    if (!owner || !repository || !chapter.file) {
        throw new Error('This chapter has no published flashcard source.');
    }
    const installed = (await getAllDecks()).find(deck => deck.id === repositoryId);
    const descriptor = (installed?.files || []).find(file => (
        (typeof file === 'string' ? file : file.path) === chapter.file
    ));
    const sha = typeof descriptor === 'string' ? null : descriptor?.sha || null;
    const markdown = await getGitHubFileContent(owner, repository, chapter.file, sha);
    const { cards } = parseDeck(markdown, chapter.file);
    return cards.map(card => {
        const identity = identifyCard(card, repositoryId);
        return {
            ...card,
            ...identity,
            source: { repo: repositoryId, file: chapter.file, sha },
            deckName: repositoryId,
            id: `${repositoryId}#${identity.hash}`
        };
    });
}
...
async function startCurriculumChapterDrill(chapter) {
    ...
    chapterBrowserReturnFocus = document.activeElement;
    const title = chapter.title || chapter.deck || chapterId;
    const deckName = curriculumDeck?.deck || deckId.split('/').pop();
    const path = `~ / ${chapter.subject} / ${deckName} / ${title}`;
    const previewGeneration = ++curriculumChapterPreviewGeneration;
    showChapterBrowser({
        title,
        path,
        summary: `${Number(chapter.card_count || 0)} cards`,
        cards: [],
        actions,
        contentInitiallyVisible: false
    });
    if (!hasGeneratedContent || !repositoryId || !chapter.file) return;
    try {
        const [cards, reviews] = await Promise.all([
            loadCurriculumChapterPreview(curriculumDeck, chapter),
            getAllReviews()
        ]);
        if (previewGeneration !== curriculumChapterPreviewGeneration
            || document.getElementById('card-browser-modal')?.classList.contains('hidden')) return;
        const reviewedHashes = new Set(reviews.map(review => review.cardHash));
        showChapterBrowser({
            title,
            path,
        ...
        if (previewGeneration !== curriculumChapterPreviewGeneration) return;
...
async function drillCurriculumChapter(chapter) {
    ...
    let repository = (await getAllDecks()).find(deck => (
        ...
        || `${subjectSlug(deck.subject)}/${String(deck.id || deck.repo || '').split('/').pop()}` === deckId
    ));
    if (!repository) {
        const repositoryId = repositoryIdForCurriculumDeck(curriculumDeck);
        if (repositoryId) {
            try {
                ({ deck: repository } = await loadRepositoryMetadata(repositoryId, { sync: true }));
            } catch (error) {
                console.error('[Curriculum] Could not add chapter repository:', error);
            }
        }
    }
    if (!repository?.id || !chapter.file) {
        alert('This chapter’s flashcards are not available in your collection yet.');
        return;
    }
    ...
    await startScopedReview(
        ...
        chapterName,
        ['home', chapter.subject, deckName, chapterName],
        [repository.id],
        [{ repo: repository.id, path: chapter.file }]
    );
}
...
async function renderCurriculumGraphCanvas(root, graph, progressStates, {
    ...
    const nodeSizing = {};
    if (compact && ranked) {
        // One full center column plus half of each neighbor and two arrow gaps.
        const nodeWidth = Math.max(60, (root.clientWidth - 2 - 40) / 2);
    ...
    for (const deck of layout.nodes) {
        ...
        node.onclick = async () => {
            if (deck.nodeType === 'subject') {
                ...
                await startCurriculumChapterDrill(deck);
            } else {
                navigateCurriculum({
                    mode: 'chapters',
                    hierarchy: 'chapter',
    ...
    const fitBounds = (bounds, { horizontal = false } = {}) => {
        ...
        const width = Math.max(1, stage.clientWidth - (compact && ranked ? 0 : padding * 2));
    ...
    const rankBounds = range => {
        ...
        return {
            x: Number.isInteger(range.layer)
                ? focalX - (compact ? layout.nodeWidth / 2 + layout.columnGap : columnStep)
                : minX,
            y: minY,
            width: Number.isInteger(range.layer)
                ? (compact ? columnStep * 2 : layout.nodeWidth + columnStep * 2)
...
function writeCurriculumHistory({ replace = false } = {}) {
    ...
    for (const key of ['study-subject', 'study-deck', 'study-chapter']) {
        url.searchParams.delete(key);
    }
...
async function renderCurriculumView(options = {}, { initialStageHeight } = {}) {
    ...
    const progressStates = new Map([
        ...curriculumDeckProgressStates(decks, cards, reviews, chapterProgress, now),
        ...curriculumChapterProgressStates(
            curriculumIndex.decks,
            decks,
            reviews,
            chapterProgress,
            now
    ...
    const activeChapterDeck = deckId ? curriculumMaps(curriculumIndex).decks.get(deckId) : null;
    ...
    const installedStudyTarget = deckId
        ...
        : subject
            ...
            : null;
    if (deckSettingsButton && activeChapterDeck) {
        deckSettingsButton.onclick = () => openCurriculumDeckActionsModal({
            deck: activeChapterDeck,
            registry: activeRegistry,
            installedRepository: installedStudyTarget
        }, deckSettingsButton);
    }
    ...
    if (mode === 'focus') {
        renderCurriculumNeighborhood(root, progressStates);
    }
...
async function handlePopState(event) {
    ...
    if (state?.mainView === 'decks') {
        await applyStudyTarget(state.studyTarget || studyTargetFromUrl());
        await showMainView('decks');
        return;
    }
