...
- **V4 — Appropriate medium.** Prefer original responsive SVG for technical
  diagrams. New generated technical figures default to editable TikZ compiled
  to ordinary SVG at authoring time. Reuse the deck's shared style and commit
  both source and output; never require TeX or TikZ compilation in the study
  client. Hand-authored SVG or another medium requires a recorded reason, such
  as an authentic photograph, a data-generated visualization, or a target TikZ
  cannot represent faithfully. Generated SVG must pass the same accessibility,
  phone-width, and stale-output checks. Use licensed photographs or raster
  illustrations only when their visual content is the authentic learning target.
