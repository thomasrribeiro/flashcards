...
- **F1 — Canonical frontmatter.** Exactly these fields in TOML `+++` frontmatter: `order` (integer, matches the filename prefix), `subject` (one of `Math`, `Physics`, `Computer Science`, `Law` — the values the app's category filter groups by), `tags` (array of kebab-case strings). Keep `[generation]` provenance tables where present (the parser ignores them). The `name` field is dead (the app overrides it with the repo name) — remove it.
