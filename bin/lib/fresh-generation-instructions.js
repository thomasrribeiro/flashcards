...
export function freshGenerationInstructions(jobType, { repair = false } = {}) {
    ...
    if (repair) {
        if (jobType !== 'curriculum-design') throw new Error('Repair is supported only for global curricula.');
        files.push(`${references}/global-curriculum-repair.md`);
    }
