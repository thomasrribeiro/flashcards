...
test('regenerates the global curriculum with model disclosure and cancellable pinned launch settings', async ({ page }, testInfo) => {
    ...
    await page.evaluate(() => localStorage.setItem('flashcards_generation_preferences_v1', JSON.stringify({
        providerId:'openai', modelId:'different-model', reasoningEffort:'max'
    })));
    await confirmAIStart(page);
...
test('curriculum Options contains only regeneration with subject entry below navigation and fits mobile', async ({ page }, testInfo) => {
    ...
    await modal.screenshot({ path: testInfo.outputPath('curriculum-options.png') });
...
test('queues a chapter-curriculum agent from an empty deck chapter viewer', async ({ page }) => {
    ...
    const create = page.locator('.curriculum-chapter-empty').getByRole('button', {
...
test('can regenerate an existing chapter curriculum without hiding the action', async ({ page }) => {
    ...
    await expect(deckCrumb).toHaveAttribute('aria-current', 'page');
    ...
    await expect(page.locator('.curriculum-neighborhood')).toBeVisible();
    const focusedDeckCrumb = page.locator('.curriculum-breadcrumb').getByRole('button', {
        name: 'elementary-algebra-and-functions', exact: true
    });
    await expect(focusedDeckCrumb).not.toHaveAttribute('aria-current', 'page');
    await focusedDeckCrumb.click();
    ...
    await regenerate.click();
    const confirmation = page.getByRole('dialog', { name: 'Regenerate chapter curriculum?' });
    await expect(confirmation).toBeVisible();
    await confirmation.getByRole('button', { name: 'Regenerate', exact: true }).click();
    await confirmAIStart(page);
...
test('opens generated chapter actions before starting its flashcards', async ({ page }) => {
    ...
    const statusBorder = await chapterNode.evaluate(node => getComputedStyle(node).borderLeftColor);
    ...
    await browser.getByRole('button', { name: 'Generate content', exact: true }).click();
    const regenerate = page.getByRole('dialog', { name: 'Regenerate chapter content?' });
    await expect(regenerate).toBeVisible();
    await regenerate.getByRole('button', { name: 'Cancel', exact: true }).click();
...
test('navigates subject graph, ranked deck layers, deck neighborhood, and chapter layers', async ({ page }, testInfo) => {
    ...
    await expect(page.locator('.curriculum-graph-node-subject')).toHaveCount(0);
    ...
    const longDeck = page.locator('.curriculum-graph-node[data-deck-id="physics/gravitational-wave-and-multimessenger-astrophysics"]');
    await expect(longDeck.locator('.curriculum-graph-node-name'))
        .toHaveText(/gravitational-wave-and-multimessenger-astrophysics/);
    await expect(longDeck.locator('.curriculum-graph-node-name')).not.toContainText('…');
    const nameLayout = await longDeck.locator('.curriculum-graph-node-name').evaluate(name => ({
    ...
    expect(nameLayout.overflow).toBe('visible');
    expect(nameLayout.scrollHeight).toBeLessThanOrEqual(nameLayout.clientHeight + 1);
    if (testInfo.project.name === 'desktop-chromium') {
        ...
        expect(visibleRanks).toEqual(['0', '1']);
    ...
    if (testInfo.project.name === 'desktop-chromium') {
        ...
        let currentRank = Number(await page.locator('.curriculum-graph-stage').getAttribute('data-scroll-layer'));
        ...
        for (let rank = tallRank; rank > 0; rank -= 1) {
            await page.getByRole('button', { name: 'Show previous dependency layer' }).click();
        }
    } else {
        await page.getByRole('button', { name: 'Show previous dependency layer' }).click();
    ...
    await expect(page).not.toHaveURL(/curriculum-layer=/);
    await expect(page.getByRole('button', { name: 'Show previous dependency layer' })).toBeDisabled();
    await page.reload();
    await expect(page.locator('.curriculum-layer-label')).toContainText('Layer 1 of');
    const layerCount = Number((await page.locator('.curriculum-layer-label').textContent()).split(' of ')[1]);
    const lastLayer = layerCount;
    for (let layer = 1; layer < lastLayer; layer += 1) {
        await page.getByRole('button', { name: 'Show next dependency layer' }).click();
    }
    await expect(page.locator('.curriculum-layer-label')).toHaveText(`Layer ${lastLayer} of ${layerCount}`);
    ...
    for (let layer = lastLayer; layer > 1; layer -= 1) {
        await page.getByRole('button', { name: 'Show previous dependency layer' }).click();
    }
    ...
    await expect(page.locator('.curriculum-neighborhood svg')).toHaveCount(0);
    ...
    if (testInfo.project.name === 'desktop-chromium') {
        ...
        const cardChrome = await page.locator('.curriculum-neighborhood').evaluate(element => {
            const style = selector => {
                const computed = getComputedStyle(element.querySelector(selector));
                return [computed.borderTopWidth, computed.borderLeftWidth, computed.padding, computed.fontFamily];
            };
            return {
                selected: style('.curriculum-selected-item'),
                related: style('.is-prerequisites .curriculum-explorer-item')
            };
        });
    ...
    const firstFocusedDeck = await selectedName.textContent();
    ...
    await expect(selectedName).toHaveText(firstFocusedDeck);
...
test('keeps every mathematics bus branch joined to its trunk', async ({ page }, testInfo) => {
    ...
    await page.locator('.curriculum-graph-node[data-deck-id="mathematics"]').click();
    await expect(page.locator('.curriculum-layer-label')).toContainText('Layer 1 of 10');
...
test('aligns another focused deck and explains an unpublished chapter plan', async ({ page }, testInfo) => {
    ...
    await deckSettings.getByRole('button', { name: /Chapter curriculum/ }).click();
    await expect(page.locator('.curriculum-chapter-empty-action')).toBeDisabled();
...
test('mobile subject layers show a full center column and half of each neighbor', async ({ page }, testInfo) => {
    ...
    for (let rank = 0; rank < count; rank += 1) {
        ...
        expect(metrics).toEqual({ ranks: [rank - 1, rank, rank + 1].filter(value => value >= 0 && value < count), visibleWidths: true, readable: true, uniform: true, aligned: true });
        await expect(page.getByRole('button', { name: 'Show previous dependency layer' })).toBeEnabled({ enabled: rank > 0 });
        await expect(page.getByRole('button', { name: 'Show next dependency layer' })).toBeEnabled({ enabled: rank < count - 1 });
        if (rank === 0 || rank === count - 1) {
            await stage.screenshot({ path: testInfo.outputPath(`boundary-layer-${rank + 1}.png`) });
        }
        if (rank === 2) {
...
for (const count of [1, 2]) {
    test(`mobile subject layers handle ${count} layers without hiding endpoints`, async ({ page }) => {
        const catalog = structuredClone(bundledCurriculum);
        catalog.subjects = catalog.subjects.filter(subject => subject.id === 'mathematics');
        catalog.decks = catalog.decks.filter(deck => deck.subject === 'mathematics').slice(0, count);
        catalog.decks.forEach((deck, index) => {
            deck.prerequisites = index ? [catalog.decks[index - 1].id] : [];
            deck.recommended_after = [];
        });
        await installGenerationAccount(page, { catalog });
        await page.locator('.curriculum-graph-node[data-deck-id="mathematics"]').click();
        const stage = page.locator('.curriculum-graph-stage');
        const previous = page.getByRole('button', { name: 'Show previous dependency layer' });
        const next = page.getByRole('button', { name: 'Show next dependency layer' });
        for (let rank = 0; rank < count; rank += 1) {
            await expect(page.locator('.curriculum-layer-label')).toHaveText(`Layer ${rank + 1} of ${count}`);
            await expect(previous).toBeEnabled({ enabled: rank > 0 });
            await expect(next).toBeEnabled({ enabled: rank < count - 1 });
            expect(await stage.locator(`[data-rank="${rank}"]`).evaluate(node => {
                const box = node.getBoundingClientRect();
                const stage = node.closest('.curriculum-graph-stage').getBoundingClientRect();
                return Math.abs((box.left + box.right - stage.left - stage.right) / 2) < 1
                    && box.left >= stage.left && box.right <= stage.right;
            })).toBe(true);
            if (rank < count - 1) await next.click();
        }
    });
}
